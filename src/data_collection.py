from __future__ import annotations

"""Collect {NodeBipartite, StrongBranchingScores} samples from Ecole.

Supports multiple MILP problem types via --problem flag:
  setcover, indepset, combauction, facility
"""

import argparse
import os
import pickle
from typing import Any, Dict, List

import numpy as np
import ecole



PROBLEM_TYPES = ["setcover", "indepset", "combauction", "facility"]


def create_instance_generator(problem: str, seed: int = 42, **kwargs):
    """Return an ecole instance generator for the given problem type.

    Extra kwargs are forwarded to the generator constructor, falling back
    to sensible defaults when omitted.
    """
    if problem == "setcover":
        generator = ecole.instance.SetCoverGenerator(
            n_rows=kwargs.get("n_rows", 500),
            n_cols=kwargs.get("n_cols", 1000),
            density=kwargs.get("density", 0.02),
            max_coef=kwargs.get("max_coef", 100),
        )
    elif problem == "indepset":
        generator = ecole.instance.IndependentSetGenerator(
            n_nodes=kwargs.get("n_nodes", 1000),
            affinity=kwargs.get("affinity", 4),
        )
    elif problem == "combauction":
        generator = ecole.instance.CombinatorialAuctionGenerator(
            n_items=kwargs.get("n_items", 100),
            n_bids=kwargs.get("n_bids", 500),
            add_item_prob=kwargs.get("add_item_prob", 0.65),
            max_n_sub_bids=kwargs.get("max_n_sub_bids", 5),
        )
    elif problem == "facility":
        generator = ecole.instance.CapacitatedFacilityLocationGenerator(
            n_customers=kwargs.get("n_customers", 200),
            n_facilities=kwargs.get("n_facilities", 100),
            continuous_assignment=kwargs.get("continuous_assignment", False),
            ratio=kwargs.get("ratio", 5.0),
        )
    else:
        raise ValueError(f"Unknown problem type: {problem!r}. Choose from {PROBLEM_TYPES}")

    generator.seed(seed)
    return generator


def create_branching_environment(use_strong_branching: bool = True) -> ecole.environment.Branching:
    if use_strong_branching:
        obs_func = (
            ecole.observation.NodeBipartite(),
            ecole.observation.StrongBranchingScores(),
        )
    else:
        obs_func = ecole.observation.NodeBipartite()

    env = ecole.environment.Branching(
        observation_function=obs_func,
        information_function={
            "nb_nodes": ecole.reward.NNodes(),
            "lp_iterations": ecole.reward.LpIterations(),
        },
    )
    return env


def get_strong_branching_action(
    action_set: np.ndarray,
    strong_branching_scores: np.ndarray,
) -> int:
    valid_scores = strong_branching_scores[action_set]
    best_local_idx = np.argmax(valid_scores)
    return int(action_set[best_local_idx])


def _ensure_finite(x: np.ndarray, *, name: str, sanitize: bool) -> np.ndarray:
    mask = ~np.isfinite(x)
    if not mask.any():
        return x
    if not sanitize:
        raise ValueError(f"{name} contains {int(mask.sum())} non-finite values")
    x = x.copy()
    x[mask] = 0.0
    return x


def extract_sample_from_observation(
    obs: Any,
    action_set: np.ndarray,
    expert_action: int,
    instance_id: int,
    step_id: int,
    sanitize_features: bool,
) -> Dict[str, Any]:
    var_feats = _ensure_finite(obs.variable_features, name="variable_features", sanitize=sanitize_features)
    con_feats = _ensure_finite(obs.row_features, name="constraint_features", sanitize=sanitize_features)

    edge_indices = obs.edge_features.indices.astype(np.int64)
    edge_values = obs.edge_features.values.flatten().astype(np.float32)

    n_vars = var_feats.shape[0]
    action_mask = np.zeros(n_vars, dtype=bool)
    action_mask[action_set] = True

    return {
        "variable_features": var_feats.astype(np.float32),
        "constraint_features": con_feats.astype(np.float32),
        "edge_indices": edge_indices,
        "edge_values": edge_values,
        "expert_action": int(expert_action),
        "action_mask": action_mask,
        "instance_id": int(instance_id),
        "step_id": int(step_id),
    }


def collect_samples(
    problem: str = "setcover",
    num_samples: int = 10000,
    seed: int = 42,
    max_steps_per_instance: int = 100,
    verbose: bool = True,
    sanitize_features: bool = False,
    **generator_kwargs,
) -> List[Dict[str, Any]]:
    generator = create_instance_generator(problem, seed=seed, **generator_kwargs)
    env = create_branching_environment(use_strong_branching=True)
    env.seed(seed)

    samples: List[Dict[str, Any]] = []
    instance_id = 0
    var_dim = None
    con_dim = None

    root_skips = 0

    if verbose:
        print(f"Collecting {num_samples} samples for problem={problem!r}")

    while len(samples) < num_samples:
        instance = next(generator)
        instance_id += 1

        obs, action_set, _, done, _ = env.reset(instance)
        if done:
            root_skips += 1
            continue
        step_id = 0

        while not done and len(samples) < num_samples and step_id < max_steps_per_instance:
            node_obs, sb_scores = obs
            if sb_scores is None:
                raise RuntimeError("Expected StrongBranchingScores but got None")

            if var_dim is None:
                var_dim = node_obs.variable_features.shape[1]
                con_dim = node_obs.row_features.shape[1]
                if verbose:
                    print(f"Feature dimensions: var={var_dim}, con={con_dim}")

            expert_action = get_strong_branching_action(action_set, sb_scores)

            sample = extract_sample_from_observation(
                node_obs,
                action_set,
                expert_action,
                instance_id,
                step_id,
                sanitize_features=sanitize_features,
            )
            samples.append(sample)

            obs, action_set, _, done, _ = env.step(expert_action)
            step_id += 1

            if verbose and len(samples) % 500 == 0:
                print(f"Collected {len(samples)}/{num_samples} samples "
                      f"(instance {instance_id}, step {step_id})")

    if verbose:
        avg = (len(samples) / instance_id) if instance_id else 0.0
        print(f"Done. samples={len(samples)} instances={instance_id} "
              f"avg_samples_per_instance={avg:.1f} root_skips={root_skips}")

    return samples


def verify_samples(samples: List[Dict[str, Any]]) -> Dict[str, Any]:
    if not samples:
        return {"error": "No samples collected"}

    var_dims = [s["variable_features"].shape[1] for s in samples]
    con_dims = [s["constraint_features"].shape[1] for s in samples]
    n_vars = [s["variable_features"].shape[0] for s in samples]
    n_cons = [s["constraint_features"].shape[0] for s in samples]
    n_edges = [s["edge_indices"].shape[1] for s in samples]

    return {
        "num_samples": len(samples),
        "var_feature_dim": var_dims[0],
        "con_feature_dim": con_dims[0],
        "var_dim_consistent": all(d == var_dims[0] for d in var_dims),
        "con_dim_consistent": all(d == con_dims[0] for d in con_dims),
        "avg_num_vars": np.mean(n_vars),
        "avg_num_cons": np.mean(n_cons),
        "avg_num_edges": np.mean(n_edges),
        "min_num_edges": min(n_edges),
        "max_num_edges": max(n_edges),
    }


def save_samples(samples: List[Dict[str, Any]], filepath: str):
    os.makedirs(os.path.dirname(filepath) or ".", exist_ok=True)
    with open(filepath, "wb") as f:
        pickle.dump(samples, f)
    print(f"Wrote {len(samples)} samples to {filepath}")


def load_samples(filepath: str) -> List[Dict[str, Any]]:
    with open(filepath, "rb") as f:
        data = pickle.load(f)
    return data


def main():
    parser = argparse.ArgumentParser(
        description="Collect MILP branching samples from various problem types",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # SetCover (default density=0.02 for ~7%% observed density)
  python data_collection.py --problem setcover
  python data_collection.py --problem setcover --density 0.01 --n_rows 1000 --n_cols 2000

  # IndependentSet (BA affinity=4; don't go below 4 — root-solves)
  python data_collection.py --problem indepset
  python data_collection.py --problem indepset --n_nodes 2000

  # CombinatorialAuction (keep add_item_prob >= 0.55 to avoid root-solves)
  python data_collection.py --problem combauction
  python data_collection.py --problem combauction --n_items 200 --n_bids 1000

  # Facility (segfaults on ecole 0.8.1 osx-arm64; use on linux/colab)
  python data_collection.py --problem facility --n_customers 200 --n_facilities 100
""",
    )
    parser.add_argument("--problem", type=str, default="setcover",
                        choices=PROBLEM_TYPES,
                        help="Problem type to generate instances for")
    parser.add_argument("--num_samples", type=int, default=10000)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--output", type=str, default=None,
                        help="Output path (default: data/<problem>_samples.pkl)")
    parser.add_argument("--max_steps", type=int, default=100,
                        help="Max branching steps per instance")
    parser.add_argument("--sanitize_features", action="store_true",
                        help="Replace non-finite features with 0 instead of raising")

    # SetCover parameters
    parser.add_argument("--n_rows", type=int, default=500)
    parser.add_argument("--n_cols", type=int, default=1000)
    parser.add_argument("--density", type=float, default=0.02)
    parser.add_argument("--max_coef", type=int, default=100)

    # IndependentSet parameters
    parser.add_argument("--n_nodes", type=int, default=1000)
    parser.add_argument("--affinity", type=int, default=4)

    # CombinatorialAuction parameters
    parser.add_argument("--n_items", type=int, default=100)
    parser.add_argument("--n_bids", type=int, default=500)
    parser.add_argument("--add_item_prob", type=float, default=0.65)
    parser.add_argument("--max_n_sub_bids", type=int, default=5)

    # CapacitatedFacilityLocation parameters
    parser.add_argument("--n_customers", type=int, default=200)
    parser.add_argument("--n_facilities", type=int, default=100)
    parser.add_argument("--continuous_assignment", action="store_true",
                        help="Use continuous [0,1] assignment vars (default: binary)")
    parser.add_argument("--ratio", type=float, default=5.0)

    args = parser.parse_args()

    output = args.output or f"data/{args.problem}_samples.pkl"

    # Forward only the relevant kwargs for the chosen problem type
    gen_kwargs = {}
    if args.problem == "setcover":
        gen_kwargs = dict(n_rows=args.n_rows, n_cols=args.n_cols,
                          density=args.density, max_coef=args.max_coef)
    elif args.problem == "indepset":
        gen_kwargs = dict(n_nodes=args.n_nodes, affinity=args.affinity)
    elif args.problem == "combauction":
        gen_kwargs = dict(n_items=args.n_items, n_bids=args.n_bids,
                          add_item_prob=args.add_item_prob,
                          max_n_sub_bids=args.max_n_sub_bids)
    elif args.problem == "facility":
        gen_kwargs = dict(n_customers=args.n_customers,
                          n_facilities=args.n_facilities,
                          continuous_assignment=args.continuous_assignment,
                          ratio=args.ratio)

    samples = collect_samples(
        problem=args.problem,
        num_samples=args.num_samples,
        seed=args.seed,
        max_steps_per_instance=args.max_steps,
        verbose=True,
        sanitize_features=args.sanitize_features,
        **gen_kwargs,
    )

    stats = verify_samples(samples)
    print("\nSample Statistics:")
    for key, value in stats.items():
        print(f"  {key}: {value}")

    save_samples(samples, output)


if __name__ == "__main__":
    main()
